...
| 5. Serving (in-cluster) | 4 | BentoML HTTP service (gRPC still beta in 1.3 - pivoted); Helm chart + ArgoCD; deployed to local kind | p95 < 50 ms at 200 RPS / replica - **✅ code-complete 2026-05-18; runtime pending (`make serve` + `make loadtest`). See [`docs/serving.md`](serving.md).** |
